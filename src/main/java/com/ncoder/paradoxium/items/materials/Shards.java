...
public final class Shards implements Listener {
    ...
    public static ItemMeta generateEffect(ItemMeta itemMeta, int amount) {
        ...
        for (int i = 0; i < amount; i++) {
            ...
            pdc.set(new NamespacedKey(Paradoxium.instance(), "effect-" + (i + 1) + "-name"), PersistentDataType.STRING, key);
            pdc.set(new NamespacedKey(Paradoxium.instance(), "effect-" + (i + 1) + "-lvl"), PersistentDataType.INTEGER, lvl);
    ...
    public static boolean isDisabled(SlimefunItemStack itemStack) {
        List<String> shards = Paradoxium.instance().getConfig().getStringList("shards.disabled");
